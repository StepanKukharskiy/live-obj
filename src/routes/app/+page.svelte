...
`);
	...
	async function appendBuiltPartScreenshot(
		...
		const normalScreenshot = captureSceneScreenshot({ frameObjectIds });
		msgs = [
			...msgs,
			{
				role: 'assistant',
		...
		if (normalScreenshot) {
			projectProcessImages = [
				...projectProcessImages,
				{
					label: `Built ${partNumber}/${partCount}: ${label}`,
					meta: 'normal render process screenshot',
					imageDataUrl: normalScreenshot
				}
			].slice(-24);
		}
		return screenshot;
...
`;
	...
	function uvHeightMapPrompt(args: { text: string; targetObjectId: string }): string {
		return [
			...
			'If the design intent includes printed, painted, inked, graphic, illustrated, decal, or patterned artwork, keep that artwork mostly neutral/low-relief in the height map; the diffuse color atlas should carry the readable artwork.',
	...
	function uvDiffuseMapPrompt(args: { text: string; targetObjectId: string }): string {
		return [
			...
			'If the design intent includes printed, painted, inked, graphic, illustrated, decal, or patterned artwork, draw it as flat readable albedo artwork on the appropriate UV islands while preserving the existing atlas layout.',
	...
	async function runUvDreamEnhanceTurn(args: {
		...
		try {
			...
			throwIfAborted(args.signal);
			diffusePngDataUrl = await imageToPngDataUrl(generatedDiffuseDataUrl);
			diffuseBmpDataUrl = await imageToColorBmpDataUrl(generatedDiffuseDataUrl);
			msgs = [
				...msgs,
				{
					role: 'assistant',
	...
	async function captureFinalRenderGalleryFrames() {
		...
		renderFrameAssets = [...nextAssets, ...renderFrameAssets].slice(0, 12);
		renderVideoShot = {
			start: nextAssets[0],
			middle: nextAssets[1],
			end: nextAssets[2],
	...
	async function sendPrompt(payload: SendPromptPayload) {
		...
		let capturedFinalRenderFrames = false;
		if (
			isIterativeEdit &&
			currentLiveObj &&
			currentLiveObj !== (latestHistoryContent ?? '').trim()
	...
	function captureSceneScreenshot(
		...
		return (
			canvasRef?.captureScreenshot({
				...
				format: 'image/jpeg',
				quality: 0.9,
				autoFrame: normalizedOptions.autoFrame,
				framePadding: normalizedOptions.framePadding,
				viewDirection: normalizedOptions.viewDirection,
