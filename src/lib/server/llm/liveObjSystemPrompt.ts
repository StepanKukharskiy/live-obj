...
- Named raw subparts: use standard OBJ g group_name face groups for meaningful editable regions inside a raw mesh, such as body, rim, legs, panels, handles, skin, glass, or trim. Then target those regions with selection-scoped post ops.
...
Supported #@post ops:
- transform selection=group_name position=[x,y,z] rotation=[rx,ry,rz] scale=[sx,sy,sz] (values may reference #@params)
- transform selection=group_name position=[x,y,z] rotation=[rx,ry,rz] scale=[sx,sy,sz] pivot=[x,y,z] (optional pivot for scale/rotation; omit selection for whole-object transform)
  Prefer position=[x,y,z]; translate=[x,y,z] is accepted only as a legacy alias.
...
- scatter count=n width=x depth=z seed=n min_distance=value jitter=value rotation=[rx,ry,rz] scale=[sx,sy,sz] position=[x,y,z] pivot=[x,y,z]
  Scatter duplicates the source raw mesh across a rectangular field on axes=xz by default. To place copies on another mesh, use target=landscape_object_name or surface=landscape_object_name, plus normal_offset=value to lift copies above the surface and optional align_to_normal=true for slope-following orientation. Use rotation=[0,rand*360,0] or rotation_y=random for random yaw, scale=[min,max] for uniform random scale, and position expressions with px, py, pz, nx, ny, nz, rand, rand_x, rand_y, rand_z, i, count, and t.
- surface_snap target=object normal_offset=value align_to_normal=true|false
- conform target=object strength=value normal_offset=value
- path_array path=object count=n spacing=value rotation_mode=tangent
- surface_array target=object spacing=value pattern=grid|hex
- orient mode=face|away target=object pivot=[x,y,z]
- clip axis=x|y|z min=value max=value
- deform selection=group_name position=[x,y,z] (omit selection for whole-object deform)
...
- subpart controls on raw mesh groups, e.g. g handle faces followed by #@post transform selection=handle scale=[handle_scale,handle_scale,handle_scale]
...
- scattered raw mesh modules with scatter; use this for fields of gates, stones, plants, columns, markers, debris, or installation pieces instead of enumerating every copy
- surface and path relationships with surface_snap, conform, path_array, surface_array, orient, and clip instead of baking many manual vertex edits
