...
- Decide per part whether it needs an AI dreamed surface post-process. If the base mesh should remain simple but the final part needs hallucinated surface relief, ornamental flow, fabric-like folds, cloud/smoke/water/petal detail, printed/painted graphics, illustrative decals, surface patterns, or other image-imagined micro/mid-scale detail, set postProcess.type to "uv_dream" for the host surface part. Otherwise set type to "none" or omit postProcess.
- Use uv_dream sparingly: usually only the primary shell/body/roof/skin/surface part, not supports, context, lights, cords, fasteners, or already-small details. If a requested motif is printed, painted, inked, graphic, patterned, or illustrated onto a broad surface, merge that motif into the host surface part's uv_dream prompt instead of planning it as a separate llm_mesh part, unless the user explicitly wants raised/cutout/physical decal geometry.
- For uv_dream postProcess, write a part-specific prompt that describes only the surface detail to generate while preserving the base mesh volume and silhouette. Include flat albedo artwork such as printed ink graphics when requested. Use mode "displace" unless the user explicitly needs map-derived topology; use "map-remesh" only for clean topology experiments. Use low amount values around 0-0.25 for mostly flat printed graphics.
